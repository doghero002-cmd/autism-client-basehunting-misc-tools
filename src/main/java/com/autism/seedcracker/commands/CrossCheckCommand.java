...
public final class CrossCheckCommand extends Command {
    ...
    private static void run(int maxDist) {
        ...
        record Pair(long tx, long tz, long bx, long bz, double dist) {}
        ...
        for (long[] t : tex) {
            for (long[] b : bed) {
                ...
                long d2 = dx * dx + dz * dz;
                if (d2 <= maxDistSq) pairs.add(new Pair(t[0], t[1], b[0], b[1], Math.sqrt(d2)));
            }
        }
        pairs.sort(java.util.Comparator.comparingDouble(Pair::dist));
        ...
        msg("§a" + pairs.size() + " agreeing pair(s) within " + maxDist + " blocks:");
        for (int i = 0; i < pairs.size() && i < MAX_REPORTED; i++) {
            Pair p = pairs.get(i);
            msg(String.format("  §aX %d Z %d §7(texture) <-> §aX %d Z %d §7(bedrock, %.1f blocks apart)",
                p.tx(), p.tz(), p.bx(), p.bz(), p.dist()));
        }
